...
import { ViewFakeAdd } from './view-fake-add/view-fake-add';

@Component({
  selector: 'app-fake-ads-components',
  imports: [
    MatTable,
    MatPaginator,
    FormsModule,
  ...
  templateUrl: './fake-ads-components.html',
  standalone: true,
  styleUrls: ['./fake-ads-components.scss']
})
export class FakeAdsComponents implements OnInit, AfterViewInit, OnDestroy {
  ...
  loadAll(): void {
    ...
    this.generalAdvertisementService.searchFakeAdvertisements(this.searchText, this.page, this.size).subscribe({
      next: (response) => {
  ...
  openViewDialog(slot: any): void {
    const dialogRef = this.matDialog.open(ViewFakeAdd, {
      width: '600px',
      disableClose: false,
      data: slot
    });
