...
import { ViewRejectedAdd } from './view-rejected-add/view-rejected-add';

@Component({
  selector: 'app-rejected-ads-components',
  imports: [
    MatTable,
    MatPaginator,
    FormsModule,
  ...
  templateUrl: './rejected-ads-components.html',
  standalone: true,
  styleUrls: ['./rejected-ads-components.scss']
})
export class RejectedAdsComponents implements OnInit, AfterViewInit, OnDestroy {
  ...
  loadAll(): void {
    ...
    this.generalAdvertisementService.searchRejectedAdvertisements(this.searchText, this.page, this.size).subscribe({
      next: (response) => {
  ...
  openViewDialog(slot: any): void {
    const dialogRef = this.matDialog.open(ViewRejectedAdd, {
      width: '600px',
      disableClose: false,
      data: slot
    });
