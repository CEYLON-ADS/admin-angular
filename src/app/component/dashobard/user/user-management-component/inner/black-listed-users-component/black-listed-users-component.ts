...
import { UpdateUser } from '../active-users-component/inner-items/update-user/update-user';
import { BlackListConformation } from '../active-users-component/inner-items/black-list-conformation/black-list-conformation';
import { ViewUser } from '../active-users-component/inner-items/view-user/view-user';
import { MatProgressSpinnerModule } from '@angular/material/progress-spinner';
import { MatIconModule } from '@angular/material/icon';

@Component({
  selector: 'app-black-listed-users-component',
  imports: [
    MatTable,
    MatPaginator,
    FormsModule,
  ...
  templateUrl: './black-listed-users-component.html',
  standalone: true,
  styleUrls: ['./black-listed-users-component.scss']
})
export class BlackListedUsersComponent implements OnInit, OnDestroy {
  ...
  loadAll(): void {
    ...
    this.userService.searchBlacklistedUsers(this.searchText, this.page, this.size).subscribe({
      next: (response) => {
  ...
  unblackListConformation(user: any): void {
    const dialogUnblackListRef = this.matDialog.open(BlackListConformation, {
      width: '500px',
      disableClose: false
    });

    dialogUnblackListRef.afterClosed().subscribe(result => {
      if (result) {
        this.unblackList(user.propertyId);
      }
    });
  }

  unblackList(userId: string): void {
    this.userService.changeUserStatus(userId, true).subscribe({
      next: (response) => {
        this.snackbarService.openSuccess(response.body.message);
        this.page = 0; // Reset to first page after unblacklisting
        this.loadAll();
      },
      error: (error) => {
        this.loading = false;
